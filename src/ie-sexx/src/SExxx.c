...
INT32 UserParaSync()
{
  ...
  ulSendLength = PackUserParaSyncReq(&stParaSyncReq, g_aucTxBuf);
  ...
  while(ucCount--)
   {
	   ...
	   //ulRecvLength1 = read_data(g_aucRxBuf, sizeof(stParaSyncRsp));
       ulRecvLength1 = read_data(g_aucRxBuf, sizeof(g_aucRxBuf));
          if( ulRecvLength1 > VALUE_0 )
		  {
			...
			if( USER_PARA_SYNC_RSP == ulMsgId )
			{
				...
				ulRecvLength2 = UnpackUserParaSyncRsp((BITS8 *)&g_aucRxBuf, (USER_PARA_SYNC_RSP_T *)&stParaSyncRsp, ulRecvLength1);
				////ROS_INFO_STREAM("ulRecvLength2: "<< ulRecvLength2 << " ." ); 
                if(  ulRecvLength2 > 0 )
				{
                    usCrcData = CRC16((BITS8 *)g_aucRxBuf, ulRecvLength1-sizeof(UINT16) );
                    ////ROS_INFO_STREAM("ulRecvLength1-sizeof(UINT16): "<< ulRecvLength1-sizeof(UINT16) << " ." ); 
				            // //ROS_INFO_STREAM("CRC16: "<< usCrcData << "   stParaSyncRsp.usCrcCode: " <<  stParaSyncRsp.usCrcCode <<"  end" );
                    //printf("CRC16: "<< usCrcData << "   stParaSyncRsp.usCrcCode: " <<  stParaSyncRsp.usCrcCode <<"  end" );
...
void StartMeasureTransmission()
{
  ...
  memset(g_aucTxBuf,0,sizeof(g_aucTxBuf));
  ulSendLength = PackStartMeasureTransmissionReq(&stMeasParaReq, g_aucTxBuf);
...
void StopMeasureTransmission()
{
  ...
  stMeasParaReq.ulMsgId = MEASURE_TRANSMISSION_REQ;
  stMeasParaReq.ucMeasureFlag = 0x00;
   ...
   while(1)
   {
    ...
    if( ulRecvLength1 > VALUE_0 )
        ...
        ///ROS_INFO_STREAM("GetLidarMeasData received");
				...
				ulSendLength2 = UnpackMeasData((BITS8 *)&g_aucRxBuf, (MEAS_DATA_RSP_T *)&g_stMeasDataRsp);

				if(  CRC16(g_aucRxBuf, ulSendLength2-sizeof(UINT16) )== g_stMeasDataRsp.usCrcCode)
				{				
	 				for(ulIndex=0;ulIndex<g_stMeasDataRsp.stMeaseDataHeadRsp.usPackMeasPointNum;ulIndex++)
					{
            ulIndex1 = g_stMeasDataRsp.stMeaseDataHeadRsp.ucPacketNo * g_stMeasDataRsp.stMeaseDataHeadRsp.usPointMaxNum+ulIndex;
						scan_msg.ranges[ ulIndex1] = g_stMeasDataRsp.stPoint[ulIndex].ulDistance * 0.0001;
            
            vpstScanData[ ulIndex1].fAngle    = (g_stMeasDataRsp.stMeaseDataHeadRsp.ucPacketNo * g_stMeasDataRsp.stMeaseDataHeadRsp.usPointMaxNum+ulIndex)*(g_stRealUserPara.stDeviceParaInfo.ulAngularResolution/10000000.0);
            vpstScanData[ ulIndex1].fDistance =  g_stMeasDataRsp.stPoint[ulIndex].ulDistance * 0.0001;
            

            //printf("ulDistance:%f\r\n",g_stMeasDataRsp.stPoint[ulIndex].ulDistance * 0.0001);
            //printf("ranges[%d]=:%f\r\n",g_stMeasDataRsp.stMeaseDataHeadRsp.ucPacketNo * g_stMeasDataRsp.stMeaseDataHeadRsp.usPointMaxNum+ulIndex,scan_msg.ranges[g_stMeasDataRsp.stMeaseDataHeadRsp.ucPacketNo * g_stMeasDataRsp.stMeaseDataHeadRsp.usPointMaxNum+ulIndex]);
...
INT32 PackUserParaSyncReq(USER_PARA_SYNC_REQ_T *vpstUserParaSyncReq, BITS8 *vpucBuff)
{
    ...
    vpstUserParaSyncReq->usCrcCode = CRC16(pucSendNum, vpucBuff-pucSendNum);
...
INT32 PackStartMeasureTransmissionReq(MEAS_DATA_HEAD_REQ_T *vpstMeasParaReq, BITS8 *vpucBuff)
{
    ...
    vpstMeasParaReq->usCrcCode = CRC16(pucSendNum, vpucBuff-pucSendNum);
